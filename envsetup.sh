...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^beast_") ; then
        BEAST_BUILD=$(echo -n $1 | sed -e 's/^beast_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $BEAST_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        BEAST_BUILD=
    fi
    export BEAST_BUILD
...
function lunch()
{
    ...
    then
        ...
        T=$(gettop)
        pushd $T > /dev/null
        $T/vendor/beast/build/tools/roomservice.py $product
        popd > /dev/null
        check_product $product
    else
        T=$(gettop)
        pushd $T > /dev/null
        $T/vendor/beast/build/tools/roomservice.py $product true
        popd > /dev/null
    fi
    if [ $? -ne 0 ]
    then
